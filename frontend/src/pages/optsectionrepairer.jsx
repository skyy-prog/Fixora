...
const OtpSectionsrepairer = () => {
  const [otp, setOtp] = useState("");
  const [message, setMessage] = useState("");
  const [loading, setLoading] = useState(false);

  const navigate = useNavigate();
  const { repairerAccountId, refreshUserInfo, setrole } = useContext(RepairContext);
  ...
  const handletoOTPsection = async (e) => {
    ...
    try {
      setLoading(true);
      if (!repairerAccountId) {
        setMessage("Please register first to get account verification OTP");
        return;
      }
      ...
      if (data.success) {
        ...
        toast.success(data.msg || "Account verified");
        navigate("/repairer/account");
      } else {
        setMessage(data.msg || "Invalid OTP");
      }
    } catch (error) {
      setMessage(error?.response?.data?.msg || "Verification failed. Try again.");
    } finally {
      setLoading(false);
    }
  };
...
export default OtpSectionsrepairer;
